...
for jar in `find $GIRAPH_HOME -type f -name "*.jar" | sort`
do
...
for jar in `find $GIRAPH_HOME -type f -name "*.jar" | sort`
do
  hadoop_libjars=$hadoop_libjars,$jar
done
